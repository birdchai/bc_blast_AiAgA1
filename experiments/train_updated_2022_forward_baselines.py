...
def split_summary(df: pd.DataFrame, splits: dict[str, pd.DataFrame]) -> pd.DataFrame:
    ...
    sparse = df[(df["year"] == 2016) & df[TARGET].notna()].copy()
    target = pd.to_numeric(sparse[TARGET], errors="coerce")
    rows.append(
        {
            "split": "reported_sparse_2016",
            "years": "2016",
            "rows": len(sparse),
            "positive_rows": int(target.fillna(0).sum()),
            "positive_rate": float(target.mean()) if len(target) else None,
            "provinces": int(sparse["province"].nunique()) if len(sparse) else 0,
            "regions": int(sparse["region"].nunique()) if len(sparse) else 0,
            "label_observed_rows": int(pd.to_numeric(sparse["label_observed"], errors="coerce").fillna(0).sum())
            if "label_observed" in sparse.columns
            else None,
            "label_observed_rate": float(pd.to_numeric(sparse["label_observed"], errors="coerce").fillna(0).mean())
            if "label_observed" in sparse.columns and len(sparse)
            else None,
        }
    )
...
def old_metric_comparison(metrics: pd.DataFrame) -> pd.DataFrame:
    rows = []
    previous_path = OUTPUT_DIR / "updated_label_model_comparison.csv"
    ...
    for _, previous_row in previous.iterrows():
        model = str(previous_row.get("model", ""))
        match = updated_test[updated_test["model"] == model]
        if match.empty:
            continue
        ...
        rows.append(
            {
                "model": model,
                "previous_split": "train_2017_2019_val_2020_test_2021",
                "new_split": "train_2017_2020_val_2021_test_2022",
                "previous_test_year": 2021,
                "new_test_year": 2022,
                "previous_f1": previous_row.get("f1"),
                "new_f1": new_row.get("f1"),
                "f1_delta_new_minus_previous": new_row.get("f1") - previous_row.get("f1"),
                "previous_precision": previous_row.get("precision"),
                "new_precision": new_row.get("precision"),
                "previous_recall": previous_row.get("recall"),
                "new_recall": new_row.get("recall"),
                "previous_roc_auc": previous_row.get("roc_auc"),
                "new_roc_auc": new_row.get("roc_auc"),
                "previous_pr_auc": previous_row.get("pr_auc"),
                "new_pr_auc": new_row.get("pr_auc"),
                "previous_false_positive": previous_row.get("false_positive"),
                "new_false_positive": new_row.get("false_positive"),
                "previous_false_negative": previous_row.get("false_negative"),
                "new_false_negative": new_row.get("false_negative"),
                "interpretation": "Both rows use corrected labels; test years differ, so this is forward-split comparison, not same-test ablation.",
            }
        )
    return pd.DataFrame(rows)
...
def main() -> None:
    ...
    outputs = {
        "updated_2022_baseline_metrics.csv": metrics,
        "updated_2022_model_comparison.csv": model_comparison,
        "updated_2022_all_predictions.csv": predictions,
        "updated_2022_test_predictions.csv": predictions[predictions["split"] == "test"].copy(),
        "updated_2022_confusion_matrix.csv": confusion,
        "updated_2022_feature_importance.csv": tree_importance.sort_values(["model", "importance"], ascending=[True, False]),
        "updated_2022_region_performance.csv": region_perf,
        "updated_2022_false_positives.csv": false_positives,
        "updated_2022_false_negatives.csv": false_negatives,
        "updated_2022_split_summary.csv": split_info,
        "updated_2022_feature_manifest.csv": manifest,
        "updated_2022_old_split_comparison.csv": old_comparison,
        "updated_2022_dnn_training_history.csv": dnn_history,
    }
